...
import {
    MARKETING_TYPE,
    PC_COMMON_MARKETING_MASK
} from "tradePublic/marketing/constants";
...
class Index extends Component {
    ...
    config = {
        ...
        usingComponents: {
            "router-view":
                "../../components/miniapp-router/router-view/router-view" // 书写第三方组件的相对路径
        }
    };
    onLoad = () => {};
    ...
    renderMarketing = type => {
        ...
        let jsx = null;
        if (type === "notice") {
            jsx = <Marketing type={MARKETING_TYPE.notice} from={from} />;
        } else if (type === "banner") {
            jsx = <Marketing type={MARKETING_TYPE.banner} from={from} />;
        } else {
            jsx = [...PC_COMMON_MARKETING_MASK, MARKETING_TYPE.midCard].map(
                item => {
                    return <Marketing type={item} from={from} />;
                }
            );
        }
        return jsx;
    ...
    render() {
        ...
        if (currentRoute && currentRoute.children) {
            currentChildren = currentRoute.children.filter(
                item => !(item.hide == true)
            );
            currentChildren.map(
                item => (item.path = item.path.replace(/\/\:.+$/, ""))
            );
        }
        let {
            haveNotice,
                ...
                <View className="nav">
                        ...
                        <View className="nav-right">
                            ...
                            >
                                ...
                                <View className="vip-time-wrapper">
                                    剩余
                                    <View className="vip-time">1234天</View>
                                </View>
                    ...
                    <View className="container">
                        ...
                        >
                            <View
                                className="item"
                                onClick={this.toggleSideBarMini}
                            >
                                <Text
                                    className={
                                        "iconfont " +
                                        (sidebarMini
                                            ? "iconfont-shouqicebianlan"
                                            : "iconfont-zhankaicebianlan")
                                    }
                                />
                            </View>
